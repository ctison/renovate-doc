...
async function generateManagers() {
  const managerIndex = require(`../renovate/dist/manager`);
  ...
  for (const [manager, definition] of managers) {
    ...
    const managerFileName = `${__dirname}/../docs/modules/manager/${manager}.md`;
...
async function generateVersioning() {
  const versionIndex = require(`../renovate/dist/versioning`);
  ...
  for (const versioning of versioningList) {
    const definition = require(`../renovate/dist/versioning/${versioning}`);
    const {
      id,
      displayName,
      urls,
...
async function generateDatasources() {
  const dsIndex = require(`../renovate/dist/datasource`);
  ...
  for (const datasource of dsList) {
    /** @type {import('../renovate/dist/datasource').Datasource}  */
    const definition = require(`../renovate/dist/datasource/${datasource}`);
    const {
      id,
      urls,
      defaultConfig
